...
class MbdControllerTest {
    ...
    when(mbdService.getMbd(any(), any()))
        .thenAnswer(
            item ->
                Mono.just(
                    ResponseEntity.ok()
                        .body(GetCartResponse.builder().checkoutRedirectUrl("testUrl").build())));
    webClient
        .post()
        .uri("/organizations/test/mbd")
        .bodyValue(
  ...
  void getPaymentReceiptsShouldRetunrContentOnValidCall() throws Exception {
    when(mbdService.getPaymentReceipts(any(), any()))
        .thenAnswer(item -> Mono.just(ResponseEntity.ok().body("ABC".getBytes())));
    webClient
        .get()
        .uri("/organizations/test/receipt/30000000001")
        .exchange()
        .expectStatus()
        .is2xxSuccessful()
